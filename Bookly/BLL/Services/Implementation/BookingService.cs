    ...
    public class BookingService : IBookingService
    {
        ...
        public async Task<Response<int>> RequestBookingAsync(int guestId, BookingRequestViewModel request)
        {
            if (request.CheckInDate.Date < DateTime.UtcNow.Date)
                return Response<int>.FailWithKey(ResponseStatus.ValidationError, "CheckInDateCannotBeInThePast");

            if (request.CheckOutDate <= request.CheckInDate)
                return Response<int>.FailWithKey(ResponseStatus.ValidationError, "CheckOutDateMustBeAfterCheckInDate");

            var listing = await _listingRepo.GetByIdAsync(request.ListingId);

            if (listing == null || !listing.IsActive)
                return Response<int>.FailWithKey(ResponseStatus.NotFound, "ListingIsUnavailable");

            if (request.NumberOfGuests > listing.MaxGuests)
                return Response<int>.FailWithKey(ResponseStatus.ValidationError, "ListingMaximumGuests", listing.MaxGuests);

            if (listing.HostId == guestId)
                return Response<int>.FailWithKey(ResponseStatus.Forbidden, "CannotBookOwnListing");
            ...
            if (overlappingBookings)
                return Response<int>.FailWithKey(ResponseStatus.Error, "DatesAlreadyBooked");
            ...
            if (saved > 0)
            {
                await _notificationService.SendNotificationAsync(
                    ...
                    new[] { listing.Title },
                    $"/Bookings/HostDetails/{booking.Id}"
                );

                return Response<int>.SuccessWithKey(booking.Id, "BookingRequestSentSuccessfully");
            }

            return Response<int>.FailWithKey(ResponseStatus.Error, "BookingSystemError");
        }
        ...
        public async Task<Response<BookingConfirmationViewModel>> GetBookingConfirmationAsync(int bookingId, int guestId)
        {
            ...
            if (booking == null)
                return Response<BookingConfirmationViewModel>.FailWithKey(ResponseStatus.NotFound, "BookingNotFound");
        ...
        public async Task<Response<MyTripsViewModel>> GetMyTripsAsync(int guestId)
        {
            ...
            {
                Upcoming = bookings
                    .Where(b => b.CheckInDate >= DateTime.UtcNow.Date &&
                                b.Status != BookingStatus.Cancelled &&
                                b.Status != BookingStatus.Rejected &&
        ...
        public async Task<Response<BookingDetailsViewModel>> GetBookingDetailsAsync(int bookingId, int userId)
        {
            ...
            if (booking == null)
                return Response<BookingDetailsViewModel>.FailWithKey(ResponseStatus.NotFound, "BookingNotFound");

            if (booking.GuestId != userId && booking.Listing.HostId != userId)
                return Response<BookingDetailsViewModel>.FailWithKey(ResponseStatus.Forbidden, "NoPermissionToViewBooking");
        ...
        public async Task<Response<bool>> RespondToBookingRequestAsync(int bookingId, int hostId, bool accept)
        {
            ...
            if (booking == null)
                return Response<bool>.FailWithKey(ResponseStatus.NotFound, "BookingNotFound");

            if (booking.Listing.HostId != hostId)
                return Response<bool>.FailWithKey(ResponseStatus.Forbidden, "YouDoNotOwnThisListing");

            if (booking.Status != BookingStatus.Pending)
                return Response<bool>.FailWithKey(ResponseStatus.ValidationError, "BookingNoLongerPending");
            ...
            if (saved > 0)
            {
                string templateKey = accept ? "BookingConfirmedNotification" : "BookingRejectedNotification";
                await _notificationService.SendNotificationAsync(
                    booking.GuestId,
                    templateKey,
                ...
                return Response<bool>.SuccessWithKey(true, accept ? "BookingConfirmedSuccessfully" : "BookingRejected");
            }

            return Response<bool>.FailWithKey(ResponseStatus.Error, "FailedToUpdateBookingStatus");
        }
        ...
        public async Task<Response<bool>> CancelBookingAsync(int bookingId, int userId)
        {
            ...
            if (booking.GuestId != userId && booking.Listing.HostId != userId)
                return Response<bool>.FailWithKey(ResponseStatus.Forbidden, "NoPermissionToCancelBooking");

            if (booking.Status == BookingStatus.Completed ||
                booking.Status == BookingStatus.Cancelled ||
                booking.Status == BookingStatus.Rejected)
            {
                return Response<bool>.FailWithKey(ResponseStatus.ValidationError, "BookingCannotBeCancelled");
            }
            ...
            if (saved > 0)
            {
                bool guestCancelled = userId == booking.GuestId;
                int targetUserId = guestCancelled ? booking.Listing.HostId : booking.GuestId;
                string templateKey = guestCancelled ? "BookingCancelledByGuestNotification" : "BookingCancelledByHostNotification";
                ...
                return Response<bool>.SuccessWithKey(true, "BookingCancelledSuccessfully");
            }

            return Response<bool>.FailWithKey(ResponseStatus.Error, "FailedToCancelBooking");
        }
