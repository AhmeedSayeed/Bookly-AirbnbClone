...
namespace PL.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public AdminController(IAdminService adminService, ICouponService couponService, IStringLocalizer<SharedResource> localizer)
        {
        ...
        [HttpGet]
        public IActionResult ViewDocument(string url, [FromServices] IWebHostEnvironment env)
        {
        ...
        {
            var response = await _adminService.ApproveVerificationAsync(id);
        ...
        {
            var response = await _adminService.RejectVerificationAsync(id);
            TempData[response.Succeeded ? "SuccessMessage" : "ErrorMessage"] = response.Message;
            return RedirectToAction(nameof(Verifications));
        }
        ...
        {
            var response = await _adminService.LockUserAsync(id, DateTimeOffset.UtcNow.AddYears(100));
            TempData[response.Succeeded ? "SuccessMessage" : "ErrorMessage"] = response.Message;
        ...
        {
            var response = await _adminService.GetAllListingsForModerationAsync();
            return View(response.Data);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ModerateListing(int id, bool isActive)
        {
            var response = await _adminService.ModerateListingAsync(id, isActive);
            TempData[response.Succeeded ? "SuccessMessage" : "ErrorMessage"] = response.Message;
